...
import { getFirestoreDB } from "./src/serverFirestore";
...
async function initFirestoreSync() {
  ...
  try {
    ...
    const requestsSnap = await firestore.collection("friendRequests").get();
    ...
    if (hasRemoteData) {
      console.log(`✅ Loaded ${db.users.length} users, ${db.messages.length} messages from Firestore.`);
      saveLocalDBOnly(db);
    } else {
    ...
    try {
      if (entityChanged) {
        ...
        if (type === 'user' && item?.id) {
          await firestore.collection("users").doc(item.id).set(item, { merge: true });
        } else if (type === 'friendRequest' && item?.id) {
          await firestore.collection("friendRequests").doc(item.id).set(item, { merge: true });
        } else if (type === 'group' && item?.id) {
          await firestore.collection("groups").doc(item.id).set(item, { merge: true });
        } else if (type === 'message' && item?.id) {
          await firestore.collection("messages").doc(item.id).set(item, { merge: true });
        }
      } else {
        // Batch backup state when multiple records update
        const batch = firestore.batch();
        for (const u of state.users.slice(0, 100)) {
          batch.set(firestore.collection("users").doc(u.id), u, { merge: true });
        }
        await batch.commit();
      }
    } catch (e) {
...
async function startServer() {
  ...
  app.post("/api/auth/status", (req, res) => {
    const { userId, status, durationMinutes } = req.body;
    const user = db.users.find((u) => u.id === userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    }
    ...
    if (status === "dnd") {
      if (durationMinutes && durationMinutes > 0) {
        user.dndUntil = Date.now() + durationMinutes * 60 * 1000;
      } else {
        user.dndUntil = null;
      }
    } else {
