...
async function initFirestoreSync() {
  ...
  try {
    ...
    const requestsSnap = await getDocs(collection(firestore, "friendRequests"));
    ...
    try {
      if (entityChanged) {
        ...
        if (type === 'user' && item?.id) {
          await setDoc(doc(firestore, "users", item.id), item, { merge: true });
        } else if (type === 'friendRequest' && item?.id) {
          await setDoc(doc(firestore, "friendRequests", item.id), item, { merge: true });
        } else if (type === 'group' && item?.id) {
          await setDoc(doc(firestore, "groups", item.id), item, { merge: true });
        } else if (type === 'message' && item?.id) {
          await setDoc(doc(firestore, "messages", item.id), item, { merge: true });
        }
      } else {
