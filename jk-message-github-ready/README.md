...
6. **로컬 파일 영속성 (Data Persistence)**
   - `data/db.json` 및 `data/uploads/` 폴더에 모든 유저, 대화, 파일이 자동으로 영구 저장됩니다.
...
이 프로젝트는 Google AI Studio에 종속되지 않도록 정리되어 있으며, 일반적인 GitHub 저장소에서 그대로 관리할 수 있습니다.

### 1. GitHub에서 새 저장소 만들기

GitHub에서 빈 repository를 만든 뒤, 저장소 주소를 복사합니다.
...
git init
git add .
git commit -m "feat: initial JK Message"
git branch -M main
git remote add origin https://github.com/사용자이름/저장소이름.git
git push -u origin main
```

이미 Git 저장소라면 `git init`은 생략하면 됩니다.
...
> ⚠️ GitHub는 소스코드를 저장하는 곳입니다. 이 앱은 Express + WebSocket 백엔드가 필요하므로 GitHub Pages만으로는 실시간 메신저 서버를 실행할 수 없습니다.
> 배포하려면 Render, Railway, Fly.io 같은 Node.js 서버 호스팅을 사용하세요.
